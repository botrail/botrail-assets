# weld-gun-c1 — 小型サーボスポットガン (C 型 / 直動)

溶接デモ用のプロセスツール (CC0-1.0)。**特定メーカー製品の複製ではない**が、
寸法と仕様は実機の公表値に一致させてある。weld-line-requests.md #2 の
「C ガン (prismatic) は次」に対応する版。
...
メーカーは無い (weld-gun-x1 の調査で OBARA / NIMAK / Milco / ARO / Electroweld を
確認済み)。そこで **外観は独自著作のまま、数値だけ公表値に合わせる**方針を採っている。

## weld-gun-x1 との使い分け

| | weld-gun-x1 (X 型) | **weld-gun-c1 (C 型)** |
|---|---|---|
| のど深さ | 400 mm | **225 mm** |
| のど開口 | 138 mm | **230 mm** |
| 関節 | revolute (0〜0.26 rad) | **prismatic (0〜0.080 m)** |
| 質量 | 95 kg | **84 kg** |
| TCP の mount からの距離 | 0.660 m | **0.468 m** |
...
|---|---|---|
| 質量 | 84 kg | 165 kg |
| J5 モーメント / 慣性 | 206 Nm / 9.9 kg·m² | 940 Nm / 89 kg·m² |
| J6 モーメント / 慣性 | 101 Nm / 4.4 kg·m² | 490 Nm / 46 kg·m² |
...
**URDF のプリミティブのみ。のど開口を保つのが目的。** builder はプリミティブを
素通しするので VHACD で開口が塞がれない。
...
```sh
python weld-gun-c1/tools/generate_meshes.py   # meshes/{body,electrode_ram}.stl
```

#!/usr/bin/env python3
"""weld-gun-xr637 の visual メッシュを生成する (再現可能な手続き的著作)。

出力: weld-gun-xr637/meshes/{body,electrode_ram}.stl (メートル単位, Z-up)

URDF の collision はプリミティブのままにし (のど開口を保つため)、visual だけを
このスクリプトのメッシュに置き換える。weld-gun-x16005 と同じ方針。

寸法の根拠は README と URDF ヘッダを参照 (Milco XR C-gun 637-12402-15 の
公表値 230 x 225 mm / 100 kVA / 1100 lbf / 84 kg に合わせてある)。

依存は trimesh のみ。実行: python weld-gun-xr637/tools/generate_meshes.py
"""
...
# トランス + 制御箱 (MFDC 100 kVA 一体型)。**マウントより後ろに置く** —
# 喉と同じ高さに座らせると、打点まわりで真っ先に当たるのがトランスになる
TF = ((-0.300, -0.040), (-0.145, 0.145), (0.020, 0.300))
SPINE = ((-0.060, 0.140), (-0.078, 0.078), (0.060, 0.250))
...
def build_body() -> trimesh.Trimesh:
    ...
    parts.append(
        member("z", [
            (TF[2][0], cx, 0.0, 0.252, 0.282),
            (TF[2][0] + 0.030, cx, 0.0, 0.260, 0.290),
            (TF[2][1] - 0.030, cx, 0.0, 0.260, 0.290),
            (TF[2][1], cx, 0.0, 0.248, 0.278),
        ], corner=0.22)
    )
    # マウントから C フレーム支柱へ渡る背骨
    parts.append(
        member("x", [
            (SPINE[0][0], 0.0, 0.155, 0.156, 0.190),
            (SPINE[0][1], 0.0, 0.155, 0.140, 0.176),
        ])
    )
    parts += rib(TF[0], (-0.142, 0.142), (TF[2][0] + 0.04, TF[2][1] - 0.04), 7)
    # 一次側ケーブルのブッシング
    parts.append(cyl_between([-0.210, 0.0, TF[2][1]], [-0.210, 0.0, TF[2][1] + 0.030], 0.026))
    # 水配管ヘッダ
    for sy in (1, -1):
        parts.append(
            cyl_between([-0.290, sy * 0.100, 0.075], [0.150, sy * 0.100, 0.075], 0.011, sections=14)
        )
    ...
    for sy in (1, -1):
        parts.append(
            cyl_between(
                [-0.040, sy * 0.075, 0.110], [0.210, sy * 0.055, 0.100], 0.018, sections=14
            )
        )

#!/usr/bin/env python3
"""weld-gun-x16005 の visual メッシュを生成する (再現可能な手続き的著作)。

出力: weld-gun-x16005/meshes/{body,electrode_arm}.stl (メートル単位, Z-up)

URDF の collision はプリミティブのままにし (のど開口を保つため)、visual だけを
このスクリプトのメッシュに置き換える。寸法定数は下の GEOMETRY ブロックに集約。
**参照実機: HERON DB6-075-X16005-00** (X 型ロボットサーボガン) — アーム寸法
384 x 160、ストローク 179 mm、加圧 3900 N、質量 95.5 kg、トランス 75 kVA を
公表値に一致させてある (README の採った値表を参照)。

依存は trimesh のみ (shapely / manifold3d 不使用 — ブーリアンは使わない)。
実行: python weld-gun-x16005/tools/generate_meshes.py
"""
...
ELECTRODE_X = 0.537  # 電極軸の X 位置 (クレビス前面 x=0.153 + のど深さ 384 mm = HERON 公表のアーム寸法)
...
# のど深さ 384 / 開口 160 (HERON 公表のアーム寸法 384 x 160)。開口はメッシュ包絡の
# 「可動アーム下面 - 固定アーム上面 @電極軸」で 160 になるよう経路を取ってある
FIXED_ARM_PATH = [(0.150, 0.288), (0.280, 0.280), (0.420, 0.275), (0.562, 0.272)]
FIXED_ARM_SECTION = [(0.078, 0.058), (0.068, 0.052), (0.058, 0.046), (0.050, 0.040)]
MOVING_ARM_PATH = [(0.150, 0.375), (0.280, 0.420), (0.430, 0.457), (0.562, 0.479)]
...
def build_body() -> trimesh.Trimesh:
    ...
    parts.append(  # 外側面 (下面) の稜線リブ — 鍛造アームの補強ウェブ
        beam(
            [(0.175, 0.262), (0.340, 0.256), (0.504, 0.253)],
...
def build_electrode_arm() -> trimesh.Trimesh:
    ...
    parts.extend(electrode(tip_moving, direction=1.0, holder_len=0.069))
    parts.append(  # 電極台座 (テーパーボス)
        loft(
            [
                ring_xy(rounded_rect(0.044, 0.044, 0.40, 5) + [ELECTRODE_X - 0.004, 0.0], 0.485),
                ring_xy(rounded_rect(0.040, 0.040, 0.60, 5) + [ELECTRODE_X - 0.002, 0.0], 0.465),
                ring_xy(ngon(0.021, 20) + [ELECTRODE_X, 0.0], 0.454),
            ]
        )
    )
    parts.append(  # 外側面 (上面) の稜線リブ
        beam(
            [(0.178, 0.416), (0.350, 0.462), (0.504, 0.492)],
            [(0.028, 0.016), (0.023, 0.013), (0.018, 0.011)],
        )
    )
    for sy in (-1, 1):  # ホースクランプ
        parts.append(box_at((0.026, 0.018, 0.016), (0.300, sy * 0.044, 0.413)))
        parts.append(box_at((0.024, 0.016, 0.014), (0.450, sy * 0.037, 0.449)))
    ...
    for sy in (-1, 1):
        parts.append(
            tube(
                [
                    [0.150, sy * 0.052, 0.394],
                    [0.280, sy * 0.058, 0.432],
                    [0.430, sy * 0.046, 0.468],
                    [0.516, sy * 0.038, 0.490],
                ],
                0.0090,
            )
        )
